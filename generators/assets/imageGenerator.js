...
const deleteAsset = sourcePath => {
  fs.unlink(sourcePath, err => {
    if (err) console.log(sourcePath, "could not be deleted or doesn't exist");
    else console.log(sourcePath, ' deleted');
  });
};
const updateInfoPlist = (infoPlist, assetName, type) => {
  ...
  if (!obj.CFBundleIcons) obj.CFBundleIcons = {};
  if (!obj.CFBundleIcons.CFBundleAlternateIcons)
    obj.CFBundleIcons.CFBundleAlternateIcons = {};
  if (!obj['CFBundleIcons~ipad']) obj['CFBundleIcons~ipad'] = {};
...
const updateAndroidManifest = (
  ...
  fs.readFile(androidManifest, function(err, data) {
    ...
    parser.parseString(data, function(err, result) {
      ...
      var activityAliases = result.manifest.application[0][
        'activity-alias'
      ].filter(ele => {
        if (ele['$'] && ele['$']['android:icon'])
          if (
            ele['$']['android:icon'].split('@mipmap/ic_launcher_')[1] ==
            assetName
          ) {
            alreadyExist = true;
            if (type === 'delete') {
...
const deleteIosAlternativeIcons = (iosFolder, projectName, assetName) => {
  updateInfoPlist(
    `${iosFolder}/${projectName}/info.plist`,
    assetName,
    'delete'
  );
  const iosIconFolder = `${iosFolder}/${projectName}/alternativeIcons`;
  Promise.all(
    iosIconSizes.map(size => {
      if (size.size == 60)
        Promise.all(
          size.multipliers.map(multiplier =>
            deleteAsset(`${iosIconFolder}/${assetName}@${multiplier}x.png`)
          )
        );
    })
  );
...
const generateAndroidAlternativeIcons = (
  ...
  updateAndroidManifest(
    `${assetsOutputPath}/android/app/src/${androidSrcDirectory}/AndroidManifest.xml`,
    assetName,
    projectName
  );
  Promise.all(
